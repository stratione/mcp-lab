...
def register(mcp: FastMCP):
    ...
    async def list_registry_images(registry: str = "dev") -> str:
        """List all images in a container registry. Registry must be 'dev', 'staging' or 'prod'. Returns image names as JSON array."""
        import json
    ...
    async def list_registries() -> str:
        """List available container registries. Returns 'dev', 'staging' and 'prod'."""
        import json
        return json.dumps(["dev", "staging", "prod"], indent=2)

    @mcp.tool()
    async def list_image_tags(image_name: str, registry: str = "dev") -> str:
        """List all tags for an image in a registry. Registry must be 'dev', 'staging' or 'prod'. Returns tags as JSON array."""
        import json
    ...
    async def get_image_manifest(image_name: str, tag: str, registry: str = "dev") -> str:
        """Get the manifest of a specific image:tag from a registry ('dev', 'staging' or 'prod'). Returns manifest digest and metadata."""
        import json
    ...
    async def tag_image(image_name: str, current_tag: str, new_tag: str, registry: str = "dev") -> str:
        """Tag an existing image with a new tag in the specified registry ('dev', 'staging' or 'prod'; default: dev). Useful for versioning (e.g. latest -> v1.0.0)."""
        import json
