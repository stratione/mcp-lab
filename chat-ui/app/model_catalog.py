...
async def ollama_models() -> list[dict[str, Any]]:
    ...
    for tag in sorted(extras):
        catalog.append({"id": tag, "label": tag, "supports_tools": True, "installed": True})
...
async def openai_models(api_key: str) -> list[dict[str, Any]]:
    if not api_key:
        return [{**m, "installed": None} for m in _OPENAI_FALLBACK]
    try:
        async with httpx.AsyncClient(timeout=5.0) as c:
            ...
            return [
                {"id": m["id"], "label": m["id"], "supports_tools": True, "installed": None}
                for m in keep
            ] or [{**m, "installed": None} for m in _OPENAI_FALLBACK]
    except Exception:
        return [{**m, "installed": None} for m in _OPENAI_FALLBACK]
...
async def anthropic_models(api_key: str) -> list[dict[str, Any]]:
    if not api_key:
        return [{**m, "installed": None} for m in _ANTHROPIC_FALLBACK]
    try:
        async with httpx.AsyncClient(timeout=5.0) as c:
            r = await c.get(
            ...
            return [
                ...
                for m in data
            ] or [{**m, "installed": None} for m in _ANTHROPIC_FALLBACK]
    except Exception:
        return [{**m, "installed": None} for m in _ANTHROPIC_FALLBACK]
...
async def google_models(api_key: str) -> list[dict[str, Any]]:
    if not api_key:
        return [{**m, "installed": None} for m in _GOOGLE_FALLBACK]
    try:
        async with httpx.AsyncClient(timeout=5.0) as c:
            r = await c.get(
            ...
            return [
                ...
                for m in keep
            ] or [{**m, "installed": None} for m in _GOOGLE_FALLBACK]
    except Exception:
        return [{**m, "installed": None} for m in _GOOGLE_FALLBACK]
...
def pretend_models() -> list[dict[str, Any]]:
    return [
        {"id": "demo", "label": "Demo (no key needed)", "supports_tools": False, "installed": True}
    ]
